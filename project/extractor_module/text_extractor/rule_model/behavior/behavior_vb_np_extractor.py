...
class BehaviorVBNPStatementExtractor(StatementExtractor):
    def __init__(self, nlp=None):
        super().__init__(nlp)
        self.extractor_name = "behavior_vb_np"
    ...
    def extract_for_A_verb_NP(self, sent_doc, subject, predicate, doc_api_name):
        statement_record_list = []
        is_than_flag = False
        has_than = False
        ...
        if object is None:
            ...
            if prep_object_list is not None and len(prep_object_list) > 0:
                ...
                prep_statement_result = self.extract_functionality_for_verb_noun_with_object(
                    sent_doc, subject, predicate, prep_object, doc_api_name)
            ...
            object_relation_tuple_result = self.extract_functionality_for_verb_noun_with_object(
                sent_doc, subject, predicate, object, doc_api_name)
